...
for (let i = 1; i <= 100; i++) {
  ...
  for (let d = 0; d < dormRooms.length; d++) {
    let name = dormRooms[d];
    let description = descriptions[Math.floor(Math.random() * Math.floor(descriptions.length))];
    let room = {
      name: name,
      description: description,
